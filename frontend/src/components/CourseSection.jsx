...
import coursesData, { courseSectionConfig } from '../data/courses';
import CourseCard from './CourseCard';

export default function CourseSection() {
  const config = courseSectionConfig || {
  ...
  return (
    <section className="py-24 sm:py-32 bg-[#f6f5ef] dark:bg-gray-900/95 border-b border-[#e9e7df] dark:border-gray-800 relative overflow-hidden transition-colors duration-500">
      
      {/* Subtle ambient lighting */}
      <div className="absolute top-10 left-1/3 w-96 h-96 bg-primary/10 dark:bg-primary/5 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-10 right-10 w-80 h-80 bg-amber-300/15 dark:bg-amber-500/5 rounded-full blur-[100px] pointer-events-none" />
        ...
        {list.length > 0 ? (
          ...
          <div className="text-center py-12 px-6 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-300 dark:border-gray-700 max-w-md mx-auto">
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Programs will be displayed here once configured in <code className="text-primary font-mono font-semibold">src/data/courses.js</code>.
            </p>
          </div>
        )}
